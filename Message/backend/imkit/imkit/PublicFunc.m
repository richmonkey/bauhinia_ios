...
+(NSTimeInterval)getStartDateInTheDayIntervalFromZero {
	...
	// 获取零点的起始时间
	NSDate *dateLeft = [IMPublicFunc getZeroDate];
	// 计算并返回两者的差额
...
+(NSTimeInterval) getDateIntervalFromZero:(NSDate *)date {
	...
	return [date timeIntervalSinceDate:[IMPublicFunc getZeroDate]];
}
// 获取调用当前的起始时间对象
+(NSTimeInterval)getStartDateInTheDayIntervalFrom1970 {
	...
	NSDate *dateRight = [calendar dateFromComponents:comps];
	// 获取零点的起始时间
	NSDate *dateLeft = [IMPublicFunc get1970Date];
	// 计算并返回两者的差额
	return [dateRight timeIntervalSinceDate:dateLeft];
}
// 获取调用当前的终止时间对象
...
+(NSTimeInterval) getDateIntervalFrom1970:(NSDate *)date {
    ...
    {
        return [date timeIntervalSinceDate:[IMPublicFunc get1970Date]];
    }
	else {
        return 0;
    }
...
+(int) getMonthDistanceCountBetween:(double)from To:(double)to {
	NSDate *dateFrom = [IMPublicFunc dateFromIntervalSince1970:from];
	NSDate *dateTo = [IMPublicFunc dateFromIntervalSince1970:to];
	
	int iYearFrom = [IMPublicFunc getYearComponentOfDate:dateFrom];
	int iYearTo = [IMPublicFunc getYearComponentOfDate:dateTo];
	int iMonthFrom = [IMPublicFunc getMonthComponentOfDate:dateFrom];
	int iMonthTo = [IMPublicFunc getMonthComponentOfDate:dateTo];
...
+(bool) isLeapYearAtCurrentDate {
	int iYear = [IMPublicFunc getYearComponentOfCurrentDate];
	if (iYear % 100 == 0) {
		return !(iYear % 400);
	}
	else {
...
+(bool) isTheDay:(NSDate*)aDay sameToThatDay:(NSDate*)thatDay{
    if (([IMPublicFunc getYearComponentOfDate:aDay] == [IMPublicFunc getYearComponentOfDate:thatDay])
        &&([IMPublicFunc getMonthComponentOfDate:aDay] == [IMPublicFunc getMonthComponentOfDate:thatDay])
        &&([IMPublicFunc getDayComponentOfDate:aDay] == [IMPublicFunc getDayComponentOfDate:thatDay])
        ) {
        return YES;
    }else{
        return  NO;
...
+(NSString *)intervalToDateString:(NSTimeInterval)interval withToday:(bool)b {
	NSDate *zeroDate = [IMPublicFunc getZeroDate];
	...
	NSString *strResult =   @"";
	if (b) {
		if ([IMPublicFunc getYearComponentOfDate:dstDate] == [IMPublicFunc getYearComponentOfDate:curDate] && \
			[IMPublicFunc getMonthComponentOfDate:dstDate] == [IMPublicFunc getMonthComponentOfDate:curDate] && \
			[IMPublicFunc getDayComponentOfDate:dstDate] == [IMPublicFunc getDayComponentOfDate:curDate]\
			) {
...
+(NSString *)intervalToDateStringFrom1970:(NSTimeInterval)interval withToday:(bool)b {
	NSDate *date1970 = [IMPublicFunc get1970Date];
...
+(char *) eraseFirstRemindString:(const char *)szRemind {
	...
	while (YES) {
		char *szComponent = [IMPublicFunc getComponentOfString:szRemind splite:',' index:i];
		if (!szComponent) {
			break;
		}
		...
		[IMPublicFunc freeComponentOfString:szComponent];
		i ++;
	}
	return szResult;
}

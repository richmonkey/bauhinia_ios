...
-(void)initializeViewComponents{
    ...
    //Master Scroll View
    self.MasterScrollView = [[SWParallaxScrollView alloc] initWithFrame:self.frame];
    ...
    [self.MasterScrollView addSubview:self.BackgroundImageView onLayer: -1];
...
-(void)buildScrollViewLeftToRight{
    ...
    for (MYIntroductionPanel *panelView in Panels) {
        ...
        [self.MasterScrollView addSubview:panelView onLayer:0];
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (self.LanguageDirection == MYLanguageDirectionLeftToRight) {
        ...
        if (self.CurrentPanelIndex == (Panels.count)) {
            ...
            if ([(id)self.theDelegate respondsToSelector:@selector(introduction:didFinishWithType:)]) {
                [self.theDelegate introduction:self didFinishWithType:MYFinishTypeSwipeOut];
            }
            ...
            if (LastPanelIndex != self.CurrentPanelIndex) { //Keeps from making the callback when just bouncing and not actually changing pages
                if ([(id)self.theDelegate respondsToSelector:@selector(introduction:didChangeToPanel:withIndex:)]) {
                    [self.theDelegate introduction:self didChangeToPanel:Panels[self.CurrentPanelIndex] withIndex:self.CurrentPanelIndex];
                }
...
-(void)skipIntroduction{
    if ([(id)self.theDelegate respondsToSelector:@selector(introduction:didFinishWithType:)]) {
        [self.theDelegate introduction:self didFinishWithType:MYFinishTypeSkipButton];
    }
